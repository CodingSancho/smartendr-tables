          ...
          {groupedOrders.map((group, index) => (
            <Card key={index}>
              <div style={{
                ...
                <SlPeople style={{position: 'absolute', left: 0, marginLeft: '10px'}} />
                <div style={{ margin: 'auto'}}>{convertToTableTitleFormat(group.table_name)}</div>
              </div>
              <div>
                {group.products.map((product, idx) => (
                  <div key={idx} style={{display: 'flex', padding: '1px 0'}}>
                    <div style={{width: '2rem', textAlign: 'center'}}>{product.quantity}</div>
                    <div style={{marginLeft: '2rem'}}>{product.name}</div>
                  </div>
                ))}
              </div>
              <div style={{
                backgroundColor: (group.table_name.startsWith("T-")) ? '#8DED8D' : '#4461E1', 
                textAlign: 'right', 
                padding: '2px 5px'
              }}>min</div>
            </Card>
